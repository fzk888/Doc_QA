...
def run_llm_MulitDocQA(query: str,only_chatKBQA: bool, prompt_template_from_user:str,temperature:float,multiple_dialogue:bool):
    ...
    if only_chatKBQA:
        ...
        if document_question_relevance(query,top_documents) == '是':
            ...
            if prompt_template_from_user:
                    ...
                    prompt_template_from_user = "您是一位由 Dana AI 开发的大型语言人工智能助手。请严格根据段落内容分点作答用户问题，请极大程度保留段落的格式与内容进行简要回答。如果涉及计算请按步骤进行计算，注意不要杜撰段落没有提及的要点，且不要重复。如果文档中出现代码相关的信息，可以将完整代码返回，如果给出的段落信息与原文无关"
    ...
    #如果不是只针对文档问答的话，就进行常规问答
        ...
        #         prompt_template_with_history = """您是一位由 Dana AI 开发的大型语言人工智能助手。请严格根据段落内容分点作答用户问题，请极大程度保留段落的格式与内容进行简要回答。如果涉及计算请按步骤进行计算，注意不要杜撰段落没有提及的要点，且不要重复。如果给出的段落信息与原文无关,# 请在相关主题后写上"信息缺失：#"以下是相关段落：段落：{top_documents}。请记住,不要一字不差的重复上下文内容。回答必须使用简体中文,回答的内容一定要清晰，分段落总结。以下是历史对话记录：{history},请参考历史对话记录。下面是用户问题：{query} 回答："""
        ...
        #                         )
        #     else:
        #         prompt_template = """您是一位由 Dana AI 开发的大型语言人工智能助手。请严格根据段落内容分点作答用户问题，请极大程度保留段落的格式与内容进行简要回答。如果涉及计算请按步骤进行计算，注意不要杜撰段落没有提及的要点，且不要重复。如果给出的段落信息与原文无关,# 请在相关主题后写上"信息缺失：#"以下是相关段落：段落：{top_documents}。请记住,不要一字不差的重复上下文内容。回答必须使用简体中文,回答的内容一定要清晰，分段落总结。下面是用户问题：{query} 回答："""
        ...
        history_str = history_list_to_str(history)
        if multiple_dialogue:
            prompt_template = """以上是历史信息{history_str}，您是一位由 Dana AI 开发的大型语言人工智能助手。热情有礼貌的和用户进行交互，根据用户的要求或提问{query},及时给用户满意的反馈和回答。回复："""
            ...
            prompt_template = """您是一位由 Dana AI 开发的大型语言人工智能助手。热情有礼貌的和用户进行交互，根据用户的要求或提问{query},及时给用户满意的反馈和回答。回复："""
