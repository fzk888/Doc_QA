...
- 多轮对话：在 `functions.run_llm_MulitDocQA` 中，若 `multiple_dialogue` 为真，将基于 `messages` 构建历史对话并加入提示词 `functions.py:371-386`。
- 流式返回：SSE 事件流封装，首段握手与尾部 `[DONE]` 结束 `app.py:541-563`。
- 请求追踪：中间件为每次请求注入 `req_id`，日志统一带 `[req:...]` 前缀，便于从入口到处理阶段串联 `app.py:69-81`、`app.py:445-594`。

## OCR 识别集成

- 开关与地址：`settings.enable_ocr_images` 控制是否识别 DOCX 图片；服务地址来自 `paths.ocr_service_url`；PDF 图像转文字由 `settings.enable_pdf_pix2text` 控制 `config.yaml:26-27,10-11`。
- DOCX 图片路径：优先从 Pandoc 生成的 Markdown 中识别 `media/` 图片并调用 OCR；若未命中，回退解压 `.docx` 的 `word/media/*` 再调用 OCR `documen_processing.py:255-296,302-348`。
- OCR 服务：`add/ocr/ocr_app.py` 暴露 `POST /detection_pic`，记录接收文件名、图片缩放与识别结果长度 `add/ocr/ocr_app.py:70-90`。

## 模块详解与原理

- 检索融合（BGE + BM25）
  - 向量检索：通过 `kb_state.kb_vectordb.as_retriever(k=4)` 获取语义相关文档块 `functions.py:140`。
  - BM25 检索：基于分词与停用词，计算稀疏匹配分数并排序 `bm25_search.py:13-60`。
  - 并行执行：两路检索并行，融合后按 `file_path` 去重，降低重排成本 `functions.py:142-170`。
  - 原理说明：向量检索在嵌入空间计算语义相似度；BM25 在词项频次、逆文档频率基础上衡量关键词匹配，两者互补（语义/词面）。

- 文档重排（BGE-reranker）
  - 候选控制：融合后截取至 `8` 个候选，避免模型过载 `functions.py:176-183`。
  - 重排模型：`DocumentReranker` 调用 `FlagReranker.compute_score` 计算 `(query, doc)` 成对得分，返回 Top-N `document_reranker.py:5-46`。
  - 结构化日志：输出候选文件名、TopN 文件名与分数 `functions.py:180-189`。
  - 原理说明：双塔/交互式重排，通过跨编码器对查询与文档做联合表征，提升相关性排序的精度。

- 向量化与索引（FAISS）
  - 文档解析：按扩展名分组顺序处理，生成 `Document` 分块 `Knowledge_based_async.py:62-181`。
  - 批量向量化：批次构建向量库并合并，适配大数据量 `Knowledge_based_async.py:184-211`。
  - 索引落盘：保存至 `faiss_index`，后续可快速加载 `Knowledge_based_async.py:220-224`、`49-60`。
  - 原理说明：将文本映射到连续向量空间，使用近似最近邻（FAISS）索引进行高效相似度检索。

- BM25 关键词检索
  - 分词与停用词：默认 `jieba`，无则退化为正则分词；停用词来自文件 `bm25_search.py:20-48`。
  - 阈值与排序：计算分数后按阈值过滤并降序排序，返回命中文档 `bm25_search.py:51-60`。
  - 原理说明：基于词项匹配的概率检索模型，强调高频关键词与罕见词的区分度。

- 多轮对话（历史拼接 + SSE）
  - 历史构建：将 `messages` 中非最后一条构建为“用户/AI”历史串，拼接提示词 `functions.py:392-405`。
  - 直接问答优先：若命中 QA 格式文档，直接从知识库输出答案并流式发送 `functions.py:430-459`。
  - SSE 流式：服务端按块发送内容，末尾 `[DONE]` 结束 `app.py:541-563`。
  - 原理说明：通过提示词加入上下文记忆，SSE 保持实时输出与低延迟体验。

- OCR 识别（DOCX/PDF）
  - DOCX 图片识别：优先识别 Pandoc Markdown 中 `media/` 图片；未命中则解压 `.docx` 的 `word/media/*` 并 OCR `documen_processing.py:254-275,298-324`。
  - PDF 兜底：解析失败或图片为主时，使用 `pix2text` 将页面图片转文字 `documen_processing.py:679-698`。
  - 服务实现：OCR 端点接收图片、缩放至上限、调用 PaddleOCR，抽取文本字段合并去重 `add/ocr/ocr_app.py:68-87`。
  - 原理说明：通过 OCR 将图片中的文本还原为可检索的字符串，提升图文混排文档的覆盖率与召回率。
