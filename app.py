...
# '''
# @app.post("/select_kb")
# async def select_kb_api(request: Request):
...
#     try:
#         history_str, total_tokens = view_history(kb_state.history)
...
# '''
# @app.get("/view_files")
# async def view_files_api(state = kb_state):
#     try:
#         files = state.kb.view_files()
#         logger.info("Files retrieved successfully")
#         return JSONResponse(status_code=200, content={"code": 200, "message": "Files retrieved successfully", "data": files})
#     except HTTPException as e:
#         logger.error(f"HTTP exception occurred: {e.detail}")
#         raise e
#     except Exception as e:
...
#     import uvicorn
#     uvicorn.run(app, host='127.0.0.1', port=7861, workers=1)
from fastapi import FastAPI, Form, File, UploadFile, HTTPException, Request, Depends
from fastapi.responses import StreamingResponse, JSONResponse, FileResponse
from fastapi.openapi.utils import get_openapi
from pydantic import BaseModel, Field
...
async def update_vectordb_api(kb_name: str = Form(...), files: List[UploadFile] = File(...)):
    ...
    return JSONResponse(status_code=200, content={"code": 200, "message": f"Knowledge base '{kb_name}' updated successfully and select", "result": "Update completed"})

@app.post("/view_guiding_questions")
async def view_guiding_questions_api(request: Request):
    await asyncio.sleep(1)  # 模拟处理时间
    return JSONResponse(status_code=200, content={"code": 200, "guiding_questions": ["Sample question 1", "Sample question 2"]})

@app.post("/remove_file")
async def remove_file_api(kb_name: str = Form(...), file_name: str = Form(...)):
    await asyncio.sleep(1)  # 模拟处理时间
    return JSONResponse(status_code=200, content={"code": 200, "message": f"File '{file_name}' removed successfully from knowledge base '{kb_name}'"})
...
async def run_llm_mulitdoc_qa_api(request: Request):
    ...
    async def output_generator():
        yield "data: Simulated response start\n\n"
        for _ in range(5):
            await asyncio.sleep(0.5)
            yield f"data: Chunk {_}\n\n"
        yield "data: [DONE]\n\n"
...
async def get_final_response(request: FinalResponseRequest):
    await asyncio.sleep(1)  # 模拟处理时间
    return JSONResponse(content={"final_response": "This is a simulated final response."})

@app.get("/display_image")
async def display_image(image_url: str):
    await asyncio.sleep(1)  # 模拟处理时间
    return JSONResponse(status_code=200, content={"message": "Image display simulated"})
...
async def run_llm_Knowlege_baes_file_QA_api(request: Request):
    ...
    async def output_generator():
        yield "data: Simulated knowledge base QA response start\n\n"
        for _ in range(3):
            await asyncio.sleep(0.5)
            yield f"data: KB QA Chunk {_}\n\n"
        yield "data: [DONE]\n\n"
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run("app:app", host='127.0.0.1', port=7861, workers=2)